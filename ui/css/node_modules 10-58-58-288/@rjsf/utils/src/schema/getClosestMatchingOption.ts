...
>(validator: ValidatorType<T, S, F>, rootSchema: S, schema?: S, formData: any = {}): number {
  let totalScore = 0;
  if (schema) {
      ...
      totalScore += reduce(
        ...
        (score, value, key) => {
          ...
          if (has(value, REF_KEY)) {
            ...
            return (
              score +
              calculateIndexScore<T, S, F>(validator, rootSchema, newSchema, formValue || {})
            );
          }
          ...
          if (value.type === 'object') {
            return (
              score +
              calculateIndexScore<T, S, F>(validator, rootSchema, value as S, formValue || {})
            );
          }
