...
export type TupleToIntersect<T extends any[]> = T extends [infer I]
  ? I
...
export type UnionToIntersect<U> = (U extends unknown ? (arg: U) => 0 : never) extends (
  arg: infer I,
) => 0
  ? I
  : never;
export type UnionLast<U> =
  UnionToIntersect<U extends unknown ? (x: U) => 0 : never> extends (x: infer L) => 0 ? L : never;
...
export type TConstructorParameters<T extends TConstructor<TSchema[], TSchema>> = TTuple<
  T['parameters']
>;
...
export type TCompositeProperties<I extends unknown, T extends readonly any[]> = Evaluate<
  T extends [infer A, ...infer B] ? TCompositeProperties<I & A, B> : I extends object ? I : {}
>;
...
export interface TConstructor<T extends TSchema[] = TSchema[], U extends TSchema = TSchema>
  extends TSchema {
  [Kind]: 'Constructor';
  static: new (
    ...param: TConstructorParameterArray<T, this['params']>
  ) => Static<U, this['params']>;
  type: 'object';
...
export type TExtends<L extends TSchema, R extends TSchema, T extends TSchema, U extends TSchema> = (
  Static<L> extends Static<R> ? T : U
) extends infer O
  ? UnionToTuple<O> extends [infer X, infer Y]
    ? TUnion<[Assert<X, TSchema>, Assert<Y, TSchema>]>
    : Assert<O, TSchema>
  : never;
export type TExcludeTemplateLiteralResult<T extends string> = TUnionResult<
  Assert<
    UnionToTuple<
      {
        [K in T]: TLiteral<K>;
  ...
  >
>;
export type TExcludeTemplateLiteral<T extends TTemplateLiteral, U extends TSchema> =
  Exclude<Static<T>, Static<U>> extends infer S
    ? TExcludeTemplateLiteralResult<Assert<S, string>>
    : never;
export type TExcludeArray<T extends TSchema[], U extends TSchema> =
  Assert<
    ...
    TSchema[]
  > extends infer R
    ? TUnionResult<Assert<R, TSchema[]>>
    : never;
export type TExclude<T extends TSchema, U extends TSchema> = T extends TTemplateLiteral
  ? TExcludeTemplateLiteral<T, U>
  : T extends TUnion<infer S>
    ? TExcludeArray<S, U>
    : T extends U
      ? TNever
      : T;
export type TExtractTemplateLiteralResult<T extends string> = TUnionResult<
  Assert<
    UnionToTuple<
      {
        [K in T]: TLiteral<K>;
  ...
  >
>;
export type TExtractTemplateLiteral<T extends TTemplateLiteral, U extends TSchema> =
  Extract<Static<T>, Static<U>> extends infer S
    ? TExtractTemplateLiteralResult<Assert<S, string>>
    : never;
export type TExtractArray<T extends TSchema[], U extends TSchema> =
  ...
  > extends infer R
    ? TUnionResult<Assert<R, TSchema[]>>
    : never;
export type TExtract<T extends TSchema, U extends TSchema> = T extends TTemplateLiteral
  ? TExtractTemplateLiteral<T, U>
  : T extends TUnion<infer S>
    ? TExtractArray<S, U>
    : T extends U
      ? T
...
export interface TFunction<T extends readonly TSchema[] = TSchema[], U extends TSchema = TSchema>
  extends TSchema {
  [Kind]: 'Function';
...
export type TKeyOf<T extends TSchema = TSchema> = (
  T extends TRecursive<infer S>
    ? TKeyOfTuple<S>
    : T extends TComposite
      ? TKeyOfTuple<T>
...
>;
export type PropertiesReduce<T extends TProperties, P extends unknown[]> = PropertiesReducer<
  T,
  {
...
export type TOmitArray<T extends TSchema[], K extends keyof any> = Assert<
  ...
  TSchema[]
>;
export type TOmitProperties<T extends TProperties, K extends keyof any> = Evaluate<
  Assert<Omit<T, K>, TProperties>
>;
export type TOmit<T extends TSchema = TSchema, K extends keyof any = keyof any> =
  T extends TRecursive<infer S>
    ? TRecursive<TOmit<S, K>>
    : T extends TComposite<infer S>
      ? TComposite<TOmitArray<S, K>>
      : T extends TIntersect<infer S>
        ? TIntersect<TOmitArray<S, K>>
        : T extends TUnion<infer S>
          ? TUnion<TOmitArray<S, K>>
          : T extends TObject<infer S>
            ? TObject<TOmitProperties<S, K>>
            : T;
...
export type TPartialArray<T extends TSchema[]> = Assert<
  ...
  TSchema[]
>;
export type TPartialProperties<T extends TProperties> = Evaluate<
  Assert<
    {
...
export type TPartial<T extends TSchema> =
  T extends TRecursive<infer S>
    ? TRecursive<TPartial<S>>
    : T extends TComposite<infer S>
      ? TComposite<TPartialArray<S>>
      : T extends TIntersect<infer S>
        ? TIntersect<TPartialArray<S>>
        : T extends TUnion<infer S>
          ? TUnion<TPartialArray<S>>
          : T extends TObject<infer S>
            ? TObject<TPartialProperties<S>>
            : T;
...
export type TPick<T extends TSchema = TSchema, K extends keyof any = keyof any> =
  T extends TRecursive<infer S>
    ? TRecursive<TPick<S, K>>
    : T extends TComposite<infer S>
      ? TComposite<TPickArray<S, K>>
      : T extends TIntersect<infer S>
        ? TIntersect<TPickArray<S, K>>
        : T extends TUnion<infer S>
          ? TUnion<TPickArray<S, K>>
          : T extends TObject<infer S>
            ? TObject<TPickProperties<S, K>>
            : T;
...
export type RecordTemplateLiteralObjectType<K extends TTemplateLiteral, T extends TSchema> = Ensure<
  TObject<
    Evaluate<{
      [_ in Static<K>]: T;
    }>
  >
>;
export type RecordTemplateLiteralType<K extends TTemplateLiteral, T extends TSchema> =
  IsTemplateLiteralFinite<K> extends true ? RecordTemplateLiteralObjectType<K, T> : TRecord<K, T>;
export type RecordUnionLiteralType<
  K extends TUnion<TLiteral<string | number>[]>,
  T extends TSchema,
> =
  Static<K> extends string
    ? Ensure<
      ...
      >
    : never;
export type RecordLiteralType<K extends TLiteral<string | number>, T extends TSchema> = Ensure<
  TObject<{
    [K2 in K['const']]: T;
  }>
>;
export type RecordNumberType<K extends TInteger | TNumber, T extends TSchema> = Ensure<
  TRecord<K, T>
>;
export type RecordStringType<K extends TString, T extends TSchema> = Ensure<TRecord<K, T>>;
export type RecordKey =
  | TUnion<TLiteral<string | number>[]>
  | TLiteral<string | number>
  | TTemplateLiteral
  | TInteger
  | TNumber
  | TString;
export interface TRecord<K extends RecordKey = RecordKey, T extends TSchema = TSchema>
  extends TSchema {
  [Kind]: 'Record';
...
export type TRequiredArray<T extends TSchema[]> = Assert<
  ...
  TSchema[]
>;
export type TRequiredProperties<T extends TProperties> = Evaluate<
  Assert<
    {
      [K in keyof T]: T[K] extends TReadonlyOptional<infer U>
        ? TReadonly<U>
...
export type TRequired<T extends TSchema> =
  T extends TRecursive<infer S>
    ? TRecursive<TRequired<S>>
    : T extends TComposite<infer S>
      ? TComposite<TRequiredArray<S>>
      : T extends TIntersect<infer S>
        ? TIntersect<TRequiredArray<S>>
        : T extends TUnion<infer S>
          ? TUnion<TRequiredArray<S>>
          : T extends TObject<infer S>
            ? TObject<TRequiredProperties<S>>
            : T;
...
export type IsTemplateLiteralFiniteCheck<T> =
  T extends TTemplateLiteral<infer U>
    ? IsTemplateLiteralFiniteArray<Assert<U, TTemplateLiteralKind[]>>
            ...
            : T extends TInteger
              ...
              : T extends TBigInt
                ...
                : T extends TLiteral
                  ? true
                  : false;
export type IsTemplateLiteralFiniteArray<T extends TTemplateLiteralKind[]> = T extends [
  infer L,
  ...infer R,
]
  ? IsTemplateLiteralFiniteCheck<L> extends false
    ? false
    : IsTemplateLiteralFiniteArray<Assert<R, TTemplateLiteralKind[]>>
  : T extends [infer L]
    ? IsTemplateLiteralFiniteCheck<L> extends false
      ? false
      : true
...
export type IsTemplateLiteralFinite<T> =
  T extends TTemplateLiteral<infer U> ? IsTemplateLiteralFiniteArray<U> : false;
          ...
          : T extends TNumber
            ...
            : T extends TBigInt
              ...
              : T extends TBoolean
                ? `${boolean}`
                : never;
export type TTemplateLiteralUnion<
  T extends TTemplateLiteralKind[],
  Acc extends string = '',
> = T extends [infer L, ...infer R]
  ? `${TTemplateLiteralConst<L, Acc>}${TTemplateLiteralUnion<Assert<R, TTemplateLiteralKind[]>, Acc>}`
  : T extends [infer L]
    ? `${TTemplateLiteralConst<L, Acc>}${Acc}`
    : Acc;
export interface TTemplateLiteral<T extends TTemplateLiteralKind[] = TTemplateLiteralKind[]>
  extends TSchema {
...
export type TTupleIntoArray<T extends TTuple<TSchema[]>> =
  T extends TTuple<infer R> ? Assert<R, TSchema[]> : never;
...
export type TUnionOfLiteral<T extends TUnion<TLiteral<string>[]>> = TUnionOfLiteralArray<
  T['anyOf']
>;
export type TUnionResult<T extends TSchema[]> = T extends []
  ? TNever
  : T extends [infer S]
    ? S
    : TUnion<T>;
export type TUnionTemplateLiteral<
  T extends TTemplateLiteral,
  S extends string = Static<T>,
> = Ensure<
  TUnionResult<
    Assert<
      UnionToTuple<
...
export declare namespace ObjectMap {
  function Map<T = TSchema>(
    schema: TSchema,
    callback: (object: TObject) => TObject,
...
export declare class StandardTypeBuilder extends TypeBuilder {
  ...
  ): RecordUnionLiteralType<K, T>;
  /** `[Standard]` Creates a Record type */
  Record<K extends TLiteral<string | number>, T extends TSchema>(
    key: K,
    schema: T,
    options?: ObjectOptions,
  ): RecordLiteralType<K, T>;
  /** `[Standard]` Creates a Record type */
  Record<K extends TTemplateLiteral, T extends TSchema>(
    key: K,
    schema: T,
    options?: ObjectOptions,
  ): RecordTemplateLiteralType<K, T>;
  /** `[Standard]` Creates a Record type */
  Record<K extends TInteger | TNumber, T extends TSchema>(
    key: K,
    schema: T,
    options?: ObjectOptions,
  ): RecordNumberType<K, T>;
  /** `[Standard]` Creates a Record type */
  Record<K extends TString, T extends TSchema>(
    key: K,
    schema: T,
    options?: ObjectOptions,
  ): RecordStringType<K, T>;
  ...
  String<Format extends string>(
    options?: StringOptions<StringFormatOption | Format>,
  ): TString<Format>;
  /** `[Standard]` Creates a template literal type */
  TemplateLiteral<T extends TTemplateLiteralKind[]>(
    kinds: [...T],
    options?: SchemaOptions,
  ): TTemplateLiteral<T>;
...
export declare class ExtendedTypeBuilder extends StandardTypeBuilder {
  ...
  ConstructorParameters<T extends TConstructor<any[], any>>(
    schema: T,
    options?: SchemaOptions,
  ...
  InstanceType<T extends TConstructor<any[], any>>(
    schema: T,
    options?: SchemaOptions,
  ): TInstanceType<T>;
