...
import type { AbortSignalWithReason, ForkedTaskExecutor, TaskResult } from '../types';
import { createListenerMiddleware, TaskAbortError } from '../index';
import { listenerCancelled, listenerCompleted, taskCancelled } from '../exceptions';
...
describe('fork', () => {
  ...
  test('forkedTask.result rejects TaskAbortError if listener is cancelled', async () => {
    ...
    expect(await deferredForkedTaskError).toEqual(new TaskAbortError(listenerCancelled));
  });
        ...
        async effect(_, listenerApi) {
          const wronglyDoNotAwaitResultOfTask = listenerApi.fork(async (forkApi) => {
            forkApi.signal.addEventListener('abort', () => {
              deferredResult.resolve((forkApi.signal as AbortSignalWithReason<unknown>).reason);
            });
          });
        },
      });
