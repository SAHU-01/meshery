import { configureStore, createAction, createSlice, isAnyOf } from '@reduxjs/toolkit';
...
import type { AbortSignalWithReason, AddListenerOverloads, TypedRemoveListener } from '../types';
...
describe('createListenerMiddleware', () => {
  ...
  let removeTypedListenerAction = removeListener as TypedRemoveListener<CounterState>;
  ...
  describe('Middleware setup', () => {
    test('Allows passing an extra argument on middleware creation', () => {
      ...
      const typedAddListener = listenerMiddleware.startListening as TypedStartListening<
        CounterState,
  ...
  describe('Subscription and unsubscription', () => {
    ...
    test.each(addListenerOptions)(
      ...
      (_, params) => {
        const effect: ListenerEffect<AnyAction, typeof store.getState, typeof store.dispatch> =
          jest.fn();
    ...
    const unforwardedActions: [string, AnyAction][] = [
      ['addListener', addListener({ actionCreator: testAction1, effect: noop })],
      ['removeListener', removeListener({ actionCreator: testAction1, effect: noop })],
    ];
    ...
    test('"can unsubscribe via middleware api', () => {
      const effect = jest.fn((action: TestAction1, api: ListenerEffectAPI<any, any>) => {
        if (action.payload === 'b') {
          api.unsubscribe();
        }
      });
        ...
        async effect(_, listenerApi) {
          const runIncrementBy = () => {
            listenerApi.dispatch(
              counterSlice.actions.incrementByAmount(listenerApi.getOriginalState().value + 2),
            );
          };
  ...
  describe('Type tests', () => {
    ...
    test('State args default to unknown', () => {
      createListenerEntry({
        predicate: (action, currentState, previousState): action is AnyAction => {
    ...
    test('Action type is inferred from args', () => {
      ...
      startListening({
        predicate: (action, currentState, previousState): action is PayloadAction<number> => {
