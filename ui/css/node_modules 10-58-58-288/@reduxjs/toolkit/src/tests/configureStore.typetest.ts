...
import type { Dispatch, AnyAction, Middleware, Reducer, Store, Action, StoreEnhancer } from 'redux';
 ...
 */
{
...

  {
    ...
    const storeWithCallback = configureStore({
      ...
      enhancers: (defaultEnhancers) =>
        defaultEnhancers.prepend(anotherPropertyStoreEnhancer).concat(somePropertyStoreEnhancer),
    });
...

  {
    ...
    type AnotherStateExtendingEnhancer = StoreEnhancer<{}, { anotherProperty: number }>;
    ...
    const storeWithCallback = configureStore({
      reducer: () => ({ aProperty: 0 }),
      enhancers: (dE) => dE.concat(someStateExtendingEnhancer, anotherStateExtendingEnhancer),
    });
 ...
 */
{
   ...
   */
  {
    const middleware = getDefaultMiddleware<StateA>().prepend((() => {}) as any as Middleware<
      (a: 'a') => 'A',
      StateA
    >);
    const store = configureStore({
   ...
   */
  {
    ...
    expectType<ReadonlyArray<typeof otherMiddleware | ThunkMiddleware | Middleware<{}>>>(
      concatenated,
    );
   ...
   */
  {
    const store = configureStore({
      ...
      middleware: (getDefaultMiddleware) =>
        getDefaultMiddleware().prepend((() => {}) as any as Middleware<(a: 'a') => 'A', StateA>),
    });
    const result1: 'A' = store.dispatch('a');
    const result2: Promise<'A'> = store.dispatch(thunkA());
    // @ts-expect-error
   ...
   */
  {
    ...
    const store = configureStore({
      reducer: reducerA,
      middleware: (getDefaultMiddleware) =>
        getDefaultMiddleware().concat(otherMiddleware).prepend(otherMiddleware2),
    });
    const result1: 'A' = store.dispatch('a');
    const result2: Promise<'A'> = store.dispatch(thunkA());
   ...
   */
  {
    const store = configureStore({
      reducer: reducerA,
      middleware: (getDefaultMiddleware) =>
        getDefaultMiddleware({ thunk: false }).prepend((() => {}) as any as Middleware<
          (a: 'a') => 'A',
          StateA
        >),
    });
