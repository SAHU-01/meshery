...
import { createAsyncThunk, unwrapResult, configureStore, createReducer } from '@reduxjs/toolkit';
...
describe('createAsyncThunk', () => {
  ...
  it('accepts arguments and dispatches the actions on resolve', async () => {
    ...
    const thunkActionCreator = createAsyncThunk('testType', async (arg: number, { requestId }) => {
      passedArg = arg;
  ...
  it('accepts arguments and dispatches the actions on reject', async () => {
    ...
    const thunkActionCreator = createAsyncThunk('testType', async (args: number, { requestId }) => {
      generatedRequestId = requestId;
      throw error;
    });
...
describe('createAsyncThunk with abortController', () => {
  const asyncThunk = createAsyncThunk('test', function abortablePayloadCreator(_: any, { signal }) {
...
test('serializeError implementation', async () => {
  ...
  const asyncThunk = createAsyncThunk<unknown, void, { serializedErrorType: string }>(
    'test',
...
describe('unwrapResult', () => {
  ...
  test('fulfilled case', async () => {
    ...
    const unwrapPromise = asyncThunk()(dispatch, getState, extra).then(unwrapResult);
  ...
  test('error case', async () => {
    ...
    const unwrapPromise2 = asyncThunk()(dispatch, getState, extra);
    await expect(unwrapPromise2.unwrap()).rejects.toEqual(miniSerializeError(error));
  });
...
describe('idGenerator option', () => {
  ...
  test('idGenerator implementation - can customizes how request IDs are generated', async () => {
    ...
    generatedRequestId = '';
    const defaultAsyncThunk = createAsyncThunk('test', async (args: void, { requestId }) => {
      generatedRequestId = requestId;
    });
...
test('`condition` will see state changes from a synchronously invoked asyncThunk', () => {
  ...
  const asyncThunk = createAsyncThunk<void, { force?: boolean }, { state: State }>(
    'test',
    onStart,
    {
