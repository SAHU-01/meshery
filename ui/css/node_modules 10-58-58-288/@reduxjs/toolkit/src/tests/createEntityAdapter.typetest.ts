...
function extractReducers<T>(adapter: EntityAdapter<T>): Omit<EntityStateAdapter<T>, 'map'> {
 ...
 */
{
  ...
  expectType<ActionCreatorWithPayload<ReadonlyArray<Entity> | Record<string, Entity>>>(
  ...
  expectType<ActionCreatorWithPayload<ReadonlyArray<EntityId>>>(slice.actions.removeMany);
  // @ts-expect-error
  ...
  expectType<ActionCreatorWithPayload<ReadonlyArray<Update<Entity>>>>(slice.actions.updateMany);
