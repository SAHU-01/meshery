 ...
 */
{
  ...
  const slice = createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(firstAction, (state, action) => state + action.payload.count);
    },
  });
 ...
 */
{
  const counter = createSlice({
    ...
    reducers: {
      ...
      decrement: (state, { payload = 1 }: PayloadAction<number | undefined>) => state - payload,
  ...
  expectType<ActionCreatorWithOptionalPayload<number | undefined>>(counter.actions.decrement);
  counter.actions.decrement();
  counter.actions.decrement(2);

  expectType<ActionCreatorWithPayload<number | number[]>>(counter.actions.multiply);
  counter.actions.multiply(2);
  counter.actions.multiply([2, 3, 4]);

  expectType<ActionCreatorWithPreparedPayload<[number, number], number>>(counter.actions.addTwo);
...

{
  ...
  function createDataSlice<T, Reducers extends SliceCaseReducers<GenericState<T>>>(
    name: string,
    reducers: ValidateSliceCaseReducers<GenericState<T>, Reducers>,
    initialState: GenericState<T>,
  ) {
