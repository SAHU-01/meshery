...
// basic usage
(async function () {
  const async = createAsyncThunk('test', (id: number) => Promise.resolve(id * 2));
 ...
 */
{
  ...
  // in that case, we have to forbid this behaviour or it will make arguments optional everywhere
  {
    const asyncThunk = createAsyncThunk('test', (arg: number | undefined, thunkApi) => 0);
    expectType<(arg?: number) => any>(asyncThunk);
    asyncThunk();
    asyncThunk(5);
    // @ts-expect-error
  ...
  {
    const asyncThunk = createAsyncThunk('test', (arg: number | void, thunkApi) => 0);
    expectType<(arg?: number) => any>(asyncThunk);
    asyncThunk();
    asyncThunk(5);
    // @ts-expect-error
...
{
  const asyncThunk = createAsyncThunk('test', (_: void, { rejectWithValue }) => {
    try {
  ...
  defaultDispatch(asyncThunk())
    .then((result) => {
      if (asyncThunk.fulfilled.match(result)) {
        expectType<ReturnType<AsyncThunkFulfilledActionCreator<boolean, void>>>(result);
        expectType<boolean>(result.payload);
        // @ts-expect-error
        expectType<any>(result.error);
      } else {
        expectType<ReturnType<AsyncThunkRejectedActionCreator<unknown, void>>>(result);
        expectType<SerializedError>(result.error);
...

{
  ...
  const shouldWork = createAsyncThunk<any, void, { serializedErrorType: Funky }>(
    'with generics',
    () => {},
    {
...
// meta return values
{
  ...
  createAsyncThunk<'ret', void, { fulfilledMeta: string }>('test', async (_, api) =>
    api.fulfillWithValue('ret' as const, ''),
  );
  ...
  createAsyncThunk<'ret', void, { rejectValue: string }>('test', async (_, api) =>
    api.rejectWithValue('ret'),
  );
  createAsyncThunk<'ret', void, { rejectValue: string; rejectedMeta: number }>('test', (_, api) =>
...

{
  ...
  const thunk = typedCAT('foo', (arg: number, api) => {
    ...
    const test2: number = api.dispatch((dispatch, getState) => {
      expectExactType<ThunkDispatch<{ foo: { value: number } }, undefined, AnyAction>>(ANY)(
        dispatch,
      );
  ...
  const thunk3 = typedCAT<number, string, { rejectValue: number }>('foo', (arg, api) => {
    expectExactType('' as string)(arg);
    // correct getState Type
    const test1: number = api.getState().foo.value;
    // correct dispatch type
