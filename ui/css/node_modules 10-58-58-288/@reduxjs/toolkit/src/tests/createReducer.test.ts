import type { CaseReducer, PayloadAction, Draft, Reducer, AnyAction } from '@reduxjs/toolkit';
...
type ToggleTodoReducer = CaseReducer<TodoState, PayloadAction<ToggleTodoPayload>>;
...
describe('createReducer', () => {
  ...
  describe('Deprecation warnings', () => {
    ...
    it('Warns about object notation deprecation, once', () => {
      ...
      expect(getLog().levels.warn).toMatch(/The object notation for `createReducer` is deprecated/);
      restore = mockConsole(createConsole());
  ...
  describe('actionMatchers argument', () => {
    ...
    test('uses the reducer of matching actionMatchers', () => {
      const reducer = createReducer(initialState, {}, [numberActionsCounter, stringActionsCounter]);
    ...
    test('can be used with string types', () => {
      const reducer = createReducer(0, (builder) =>
        builder
          ...
          .addCase(
            'decrement',
            (state, action: { type: 'decrement'; payload: number }) => state - action.payload,
          ),
      );
      expect(reducer(0, increment(5))).toBe(5);
      expect(reducer(5, decrement(5))).toBe(0);
