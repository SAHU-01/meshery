...
describe('isAnyOf', () => {
  ...
  it('returns true only if any matchers match (thunk action creators)', () => {
    ...
    expect(isAnyOf(thunkA.pending, thunkA.rejected, thunkB.fulfilled)(action)).toEqual(false);
  });
...
describe('isAllOf', () => {
  it('returns true only if all matchers match', () => {
    ...
    expect(isAllOf(thunkA.fulfilled, isActionSpecial)(specialThunkAction)).toBe(true);

    const ordinaryThunkAction = thunkA.fulfilled('ORDINARY', 'fakeRequestId');

    expect(isAllOf(thunkA.fulfilled, isActionSpecial)(ordinaryThunkAction)).toBe(false);
  });
});
