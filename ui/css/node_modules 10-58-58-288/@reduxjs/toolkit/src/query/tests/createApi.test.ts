...
describe('endpoint definition typings', () => {
  ...
  describe('enhancing endpoint definitions', () => {
    ...
    test('updated transform response types', async () => {
      ...
      type M1Definition = OverrideResultType<Definitions['mutation1'], Transformed>;
      ...
      const queryResponse = await storeRef.store.dispatch(enhancedApi.endpoints.query1.initiate());
      expect(queryResponse.data).toEqual({ value: 'transformed' });
      expectType<Transformed | Promise<Transformed> | undefined>(queryResponse.data);
...
describe('additional transformResponse behaviors', () => {
  ...
  test('transformResponse transforms a response from a mutation with an error', async () => {
    const result = await storeRef.store.dispatch(api.endpoints.mutationWithError.initiate({}));
...
describe('custom serializeQueryArgs per endpoint', () => {
  const customArgsSerializer: SerializeQueryArgs<number> = ({ endpointName, queryArgs }) =>
  ...
  const api = createApi({
    baseQuery: fetchBaseQuery({ baseUrl: 'https://example.com' }),
    serializeQueryArgs: ({ endpointName, queryArgs }) => `base-${endpointName}-${queryArgs}`,
    endpoints: (build) => ({
  ...
  it('Works via createApi', async () => {
    await storeRef.store.dispatch(api.endpoints.queryWithNoSerializer.initiate(99));

    expect(serializer1).toHaveBeenCalledTimes(0);

    await storeRef.store.dispatch(api.endpoints.queryWithCustomSerializer.initiate(42));

    expect(serializer1).toHaveBeenCalled();

    expect(storeRef.store.getState().api.queries['base-queryWithNoSerializer-99']).toBeTruthy();

    expect(storeRef.store.getState().api.queries['queryWithCustomSerializer-42']).toBeTruthy();
  });
  ...
  it('Works via injectEndpoints', async () => {
    ...
    expect(
      storeRef.store.getState().api.queries['injectedQueryWithCustomSerializer-5'],
    ).toBeTruthy();
  });
  ...
  test('Serializes a returned object for query args', async () => {
    ...
    expect(
      storeRef.store.getState().api.queries['queryWithCustomObjectSerializer({"id":42})'],
    ).toBeTruthy();
  });
  ...
  test('Serializes a returned primitive for query args', async () => {
    ...
    expect(
      storeRef.store.getState().api.queries['queryWithCustomNumberSerializer(42)'],
    ).toBeTruthy();
  });
