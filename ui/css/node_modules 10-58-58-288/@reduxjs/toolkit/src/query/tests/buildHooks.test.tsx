...
import type { UseMutation, UseQuery } from '@reduxjs/toolkit/dist/query/react/buildHooks';
import { createApi, fetchBaseQuery, QueryStatus, skipToken } from '@reduxjs/toolkit/query/react';
...
describe('hooks tests', () => {
  describe('useQuery', () => {
    ...
    test('useQuery hook sets isLoading and isFetching to the correct states', async () => {
      ...
      function User() {
        ...
        const { isLoading, isFetching, refetch } = api.endpoints.getUser.useQuery(22, {
          skip: value < 1,
        });
        refetchMe = refetch;
        return (
    ...
    test(`useQuery refetches when query args object changes even if serialized args don't change`, async () => {
      function ItemList() {
        ...
        const renderedItems = data.map((item) => <li key={item.id}>ID: {item.id}</li>);
        return (
          <div>
    ...
    test('useQuery refetch method returns a promise that resolves with the result', async () => {
      const { result } = renderHook(() => api.endpoints.getIncrementedAmount.useQuery(), {
        wrapper: storeRef.wrapper,
      });
    ...
    test('Hook subscriptions are properly cleaned up when changing skip back and forth', async () => {
      ...
      const { result, rerender, unmount } = renderHook(
        ([arg, options]: Parameters<typeof pokemonApi.useGetPokemonByNameQuery>) =>
  ...
  describe('useLazyQuery', () => {
    ...
    test('useLazyQuery accepts updated subscription options and only dispatches updateSubscriptionOptions when values are updated', async () => {
      ...
      expect(actions.filter(api.internalActions.updateSubscriptionOptions.match)).toHaveLength(1);
    });
    ...
    test('useLazyQuery accepts updated args and unsubscribes the original query', async () => {
      ...
      expect(actions.filter(api.internalActions.unsubscribeQueryResult.match)).toHaveLength(0);
      ...
      expect(actions.filter(api.internalActions.unsubscribeQueryResult.match)).toHaveLength(1);

      fireEvent.click(screen.getByTestId('fetchUser1'));

      expect(actions.filter(api.internalActions.unsubscribeQueryResult.match)).toHaveLength(2);

      // we always unsubscribe the original promise and create a new one
      fireEvent.click(screen.getByTestId('fetchUser1'));
      expect(actions.filter(api.internalActions.unsubscribeQueryResult.match)).toHaveLength(3);

      unmount();

      // We unsubscribe after the component unmounts
      expect(actions.filter(api.internalActions.unsubscribeQueryResult.match)).toHaveLength(4);
    });
  ...
  describe('useQuery and useMutation invalidation behavior', () => {
    ...
    test('initially failed useQueries that provide an tag will refetch after a mutation invalidates it', async () => {
      ...
      await waitFor(() =>
        expect(screen.getByTestId('user').textContent).toBe(JSON.stringify(checkSessionData)),
      );
...
describe('hooks with createApi defaults set', () => {
  ...
  describe('selectFromResult (mutation) behavior', () => {
    ...
    it('causes rerenders when only selected data changes', async () => {
      ...
      fireEvent.click(screen.getByTestId('incrementButton'));
      await waitFor(() =>
        expect(screen.getByTestId('data').textContent).toBe(JSON.stringify({ amount: 1 })),
      );
      expect(getRenderCount()).toBe(3);

      fireEvent.click(screen.getByTestId('incrementButton'));
      await waitFor(() =>
        expect(screen.getByTestId('data').textContent).toBe(JSON.stringify({ amount: 2 })),
      );
      expect(getRenderCount()).toBe(5);
    });
...
// type tests:
{
  ...
  const fakeQuery = ANY as UseQuery<typeof api.endpoints.getUser.Types.QueryDefinition>;
  expectExactType(fakeQuery)(api.endpoints.getUser.useQuery);

  // UseMutation type can be used to recreate the hook type
  const fakeMutation = ANY as UseMutation<typeof api.endpoints.updateUser.Types.MutationDefinition>;
