...
describe('buildSlice', () => {
  ...
  it('replaces previous tags with new provided tags', async () => {
    ...
    expect(api.util.selectInvalidatedBy(storeRef.store.getState(), ['SUCCEED'])).toHaveLength(1);
    expect(api.util.selectInvalidatedBy(storeRef.store.getState(), ['FAILED'])).toHaveLength(0);
    ...
    expect(api.util.selectInvalidatedBy(storeRef.store.getState(), ['SUCCEED'])).toHaveLength(0);
    expect(api.util.selectInvalidatedBy(storeRef.store.getState(), ['FAILED'])).toHaveLength(1);
  });
});
  ...
  it('Allows returning a different value from `merge`', async () => {
    ...
    const api = createApi({
      ...
      endpoints: (build) => ({
        getTodos: build.query<Todo[], void>({
          async queryFn() {
            const item = firstQueryFnCall ? { id: '0', text: 'A' } : { id: '1', text: 'B' };
