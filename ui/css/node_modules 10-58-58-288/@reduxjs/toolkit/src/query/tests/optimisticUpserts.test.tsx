...
describe('upsertQueryData', () => {
  ...
  test('does update non-existing values', async () => {
    ...
    const { result, rerender } = renderHook(() => api.endpoints.post.useQuery('4'), {
      wrapper: storeRef.wrapper,
    });
  ...
  test('upsert while a normal query is running (success)', async () => {
    ...
    const upsertRes = storeRef.store.dispatch(api.util.upsertQueryData('post', '3', upsertedData));
...
describe('full integration', () => {
  ...
  test('Interop with in-flight requests', async () => {
    await act(async () => {
      const fetchRes = storeRef.store.dispatch(api.endpoints.post2.initiate('3'));
