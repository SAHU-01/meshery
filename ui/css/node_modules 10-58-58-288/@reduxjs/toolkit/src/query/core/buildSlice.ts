...
import type { AssertTagTypes, EndpointDefinitions, QueryDefinition } from '../endpointDefinitions';
...
export function buildSlice({
  ...
  queryThunk,
  mutationThunk,
  context: { endpointDefinitions: definitions, apiUid, extractRehydrationInfo, hasRehydrationInfo },
  assertTagType,
  config,
}: {
  reducerPath: string;
  ...
  const querySlice = createSlice({
    ...
    reducers: {
      removeQueryResult: {
        reducer(draft, { payload: { queryCacheKey } }: PayloadAction<QuerySubstateIdentifier>) {
      ...
      queryResultPatched(
        ...
        }: PayloadAction<QuerySubstateIdentifier & { patches: readonly Patch[] }>,
      ) {
              ...
              substate.data =
                (definitions[meta.arg.endpointName].structuralSharing ?? true)
                  ? copyWithStructuralSharing(
                      isDraft(substate.data) ? original(substate.data) : substate.data,
                      payload,
                    )
                  : payload;
            }
  ...
  const subscriptionSlice = createSlice({
    ...
    reducers: {
      ...
      unsubscribeQueryResult(d, a: PayloadAction<{ requestId: string } & QuerySubstateIdentifier>) {
        // Dummy
      },
    ...
    reducers: {
      middlewareRegistered(state, { payload }: PayloadAction<string>) {
        state.middlewareRegistered =
          state.middlewareRegistered === 'conflict' || apiUid !== payload ? 'conflict' : true;
      },
    },
    extraReducers: (builder) => {
      builder
  ...
  const combinedReducer = combineReducers<CombinedQueryState<any, string, string>>({
