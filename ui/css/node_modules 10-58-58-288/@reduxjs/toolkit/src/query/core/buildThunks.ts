...
import type { AnyAction, ThunkAction, ThunkDispatch, AsyncThunk } from '@reduxjs/toolkit';
...
>({
  ...
  const updateQueryData: UpdateQueryDataThunk<EndpointDefinitions, State> =
    (endpointName, args, updateRecipe) => (dispatch, getState) => {
      const currentState = (api.endpoints[endpointName] as ApiEndpointQuery<any, any>).select(args)(
        getState(),
      );
  ...
  > = async (
    arg,
    { signal, abort, rejectWithValue, fulfillWithValue, dispatch, getState, extra },
  ) => {
    const endpointDefinition = endpointDefinitions[arg.endpointName];

    try {
      let transformResponse: (baseQueryReturnValue: any, meta: any, arg: any) => any =
      ...
      if (catchedError instanceof HandledError) {
        let transformErrorResponse: (baseQueryReturnValue: any, meta: any, arg: any) => any =
          defaultTransformResponse;
...
In the case of an unhandled error, no tags will be "provided" or "invalidated".`,
  ...
  function isForcedQuery(arg: QueryThunkArg, state: RootState<any, string, ReducerPath>) {
    ...
    if (refetchVal) {
      ...
      return (
        refetchVal === true || (Number(new Date()) - Number(fulfilledVal)) / 1000 >= refetchVal
      );
    }
    return false;
  }
  ...
  const hasMaxAge = (options: any): options is { ifOlderThan: false | number } =>
  ...
  const prefetch =
    ...
    (dispatch: ThunkDispatch<any, any, any>, getState: () => any) => {
      ...
      const queryAction = (force: boolean = true) =>
        (api.endpoints[endpointName] as ApiEndpointQuery<any, any>).initiate(arg, {
          forceRefetch: force,
        });
      const latestStateValue = (api.endpoints[endpointName] as ApiEndpointQuery<any, any>).select(
        arg,
      )(getState());
      ...
      if (force) {
        ...
        const shouldRetrigger =
          (Number(new Date()) - Number(new Date(lastFulfilledTs))) / 1000 >= maxAge;
        if (shouldRetrigger) {
          dispatch(queryAction());
        }
      } else {
