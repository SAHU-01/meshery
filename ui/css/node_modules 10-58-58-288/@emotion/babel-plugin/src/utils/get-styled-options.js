...
export let getStyledOptions = (t: *, path: *, state: *) => {
  ...
  if (!knownProperties.has('target')) {
    prodProperties.push(
      t.objectProperty(t.identifier('target'), t.stringLiteral(getTargetClassName(state, t))),
    );
  }
  ...
  if (optionsArgument) {
    ...
    if (
      ...
      t.buildMatchMemberExpression('process.env.NODE_ENV')(optionsArgument.test.left)
    ) {
