...
import { appendStringReturningExpressionToArguments, joinStringLiterals } from './strings';
...
export let transformExpressionWithStyles = ({
  ...
  if (t.isCallExpression(path)) {
    const canAppendStrings = path.node.arguments.every((arg) => arg.type !== 'SpreadElement');
    ...
    if (path.node.arguments.length === 1 && t.isStringLiteral(path.node.arguments[0])) {
      ...
      let res = serializeStyles([
        `${cssString}${label && autoLabel === 'always' ? `;label:${label};` : ''}`,
      ]);
      ...
      if (!state.emotionStringifiedCssId) {
        ...
        const cssObjectToString = t.functionDeclaration(
          uid,
          [],
          t.blockStatement([t.returnStatement(t.stringLiteral(CSS_OBJECT_STRINGIFIED_ERROR))]),
        );
      ...
      let devNode = t.objectExpression(
        [
          t.objectProperty(t.identifier('name'), t.stringLiteral(res.name)),
          t.objectProperty(t.identifier('styles'), t.stringLiteral(res.styles)),
          sourceMap && t.objectProperty(t.identifier('map'), t.stringLiteral(sourceMap)),
          t.objectProperty(t.identifier('toString'), t.cloneNode(state.emotionStringifiedCssId)),
        ].filter(Boolean),
      );
