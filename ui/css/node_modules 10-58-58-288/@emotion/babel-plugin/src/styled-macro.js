...
export let styledTransformer = ({
  ...
  let getStyledIdentifier = () => {
    if (
      !styledBaseImport ||
      (styledBaseImport[0] === importSource && styledBaseImport[1] === importSpecifierName)
    ) {
  ...
  if (t.isMemberExpression(reference.parent) && reference.parent.computed === false) {
    if (
      ...
      reference.parentPath.replaceWith(
        t.callExpression(getStyledIdentifier(), [t.stringLiteral(reference.parent.property.name)]),
      );
    } else {
...
export let createStyledMacro = ({
  ...
  createTransformerMacro(
    {
      default: [styledTransformer, { styledBaseImport: [importSource, baseImportName], isWeb }],
    },
