...
function Binary(node, parent) {
  ...
  if (parentType === 'BinaryExpression' || parentType === 'LogicalExpression') {
    ...
    if (
      (parentPos === nodePos && parent.right === node && parentType !== 'LogicalExpression') ||
      parentPos > nodePos
    ) {
      return true;
    }
...
function UnaryLike(node, parent) {
  return (
    hasPostfixPart(node, parent) ||
    (isBinaryExpression(parent) && parent.operator === '**' && parent.left === node) ||
    isClassExtendsClause(node, parent)
  );
}
...
function isFirstInContext(printStack, checkParam) {
  ...
  while (i >= 0) {
    const parentType = parent.type;
    if (
      (expressionStatement && parentType === 'ExpressionStatement' && parent.expression === node) ||
      (exportDefault && parentType === 'ExportDefaultDeclaration' && node === parent.declaration) ||
      (arrowBody && parentType === 'ArrowFunctionExpression' && parent.body === node) ||
    ...
    if (
      ...
      ((hasPostfixPart(node, parent) && parentType !== 'NewExpression') ||
        (parentType === 'SequenceExpression' && parent.expressions[0] === node) ||
        (parentType === 'UpdateExpression' && !parent.prefix) ||
        (parentType === 'ConditionalExpression' && parent.test === node) ||
        ((parentType === 'BinaryExpression' || parentType === 'LogicalExpression') &&
          parent.left === node) ||
