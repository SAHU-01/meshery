...
function constructExportCall(
  ...
  if (!exportStarTarget) {
    if (exportNames.length === 1) {
      ...
      statements.push(
        _core.types.expressionStatement(
          _core.types.callExpression(exportIdent, [_core.types.objectExpression(objectProperties)]),
        ),
      );
    }
  } else {
    ...
    statements.push(
      _core.types.expressionStatement(
        _core.types.callExpression(exportIdent, [_core.types.identifier(exportObj)]),
      ),
    );
  }
  return statements;
    ...
    'AssignmentExpression|UpdateExpression'(path) {
      ...
      if (isPostUpdateExpression) {
        node = _core.types.binaryExpression(
          node.operator[0],
          _core.types.unaryExpression('+', _core.types.cloneNode(node.argument)),
    ...
    visitor: {
      CallExpression(path, state) {
        if (_core.types.isImport(path.node.callee)) {
          ...
          path.replaceWith(
            (0, _helperModuleTransforms.buildDynamicImport)(path.node, false, true, (specifier) =>
