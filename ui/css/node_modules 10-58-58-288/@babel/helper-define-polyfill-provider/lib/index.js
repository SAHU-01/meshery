...
function instantiateProvider(factory, options, missingDependencies, dirname, debugLog, babelApi) {
  ...
  const getUtils = (0, _utils.createUtilsGetter)(
    new _importsCache.default((moduleName) => deps.resolve(dirname, moduleName, absoluteImports)),
  ...
  const api = {
    ...
    shouldInjectPolyfill(name) {
      ...
      if (!polyfillsNames.has(name)) {
        console.warn(
          `Internal error in the ${provider.name} provider: ` + `unknown polyfill "${name}".`,
        );
      }
    ...
    debug(name) {
      ...
      (_debugLog$polyfillsSu = (_debugLog = debugLog()).polyfillsSupport) != null
        ? _debugLog$polyfillsSu
        : (_debugLog.polyfillsSupport = polyfillsSupport);
    },
    ...
    assertDependency(name, version = '*') {
      ...
      const found = missingDependencies.all
        ? false
        : mapGetOr(depsCache, `${name} :: ${dirname}`, () => deps.has(dirname, name));
...
function definePolyfillProvider(factory) {
  return (0, _helperPluginUtils.declare)((babelApi, options, dirname) => {
    ...
    let debugLog;
    const missingDependencies = (0, _normalizeOptions.applyMissingDependenciesDefaults)(
      options,
      babelApi,
    );
    const { debug, method, targets, provider, callProvider } = instantiateProvider(
      factory,
      options,
      missingDependencies,
      dirname,
    ...
    return {
      ...
      post() {
        ...
        for (const name of debugLog.polyfills) {
          ...
          if (
            (_debugLog$polyfillsSu2 = debugLog.polyfillsSupport) != null &&
            _debugLog$polyfillsSu2[name]
          ) {
            const filteredTargets = (0, _helperCompilationTargets.getInclusionReasons)(
              name,
              targets,
