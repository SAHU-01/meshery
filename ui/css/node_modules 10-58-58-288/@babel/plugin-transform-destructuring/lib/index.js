...
function unshiftForXStatementBody(statementPath, newStatements) {
  ...
  const hasShadowedBlockScopedBindings = Object.keys(bodyScopeBindings).some((name) =>
...
class DestructuringTransformer {
  ...
  pushAssignmentPattern({ left, right }, valueRef) {
    ...
    if (core.types.isPattern(left)) {
      ...
      if (this.kind === 'const' || this.kind === 'let' || this.kind === 'using') {
        ...
        patternId = tempId;
        node = core.types.expressionStatement(
          core.types.assignmentExpression('=', core.types.cloneNode(tempId), tempConditional),
        );
      }
  ...
  pushArrayPattern(pattern, arrayRef) {
    if (arrayRef === null) {
      this.nodes.push(
        core.types.expressionStatement(
          core.types.callExpression(this.addHelper('objectDestructuringEmpty'), []),
        ),
      );
      return;
    }
  ...
  if (keys.length === 0) {
    ...
    if (!allLiteral) {
      keyExpression = core.types.callExpression(
        core.types.memberExpression(keyExpression, core.types.identifier('map')),
...
function convertVariableDeclaration(
  ...
  for (let i = 0; i < node.declarations.length; i++) {
    ...
    if (core.types.isPattern(pattern)) {
      ...
      nodes.push(
        core.types.inherits(destructuring.buildVariableAssignment(pattern, patternId), declar),
      );
    }
  }
  let tail = null;
...
function convertAssignmentExpression(
  ...
  let ref;
  if (
    (!parentPath.isExpressionStatement() && !parentPath.isSequenceExpression()) ||
    path.isCompletionRecord()
  ) {
    ref = scope.generateUidIdentifierBasedOnNode(node.right, 'ref');
    nodes.push(
      core.types.variableDeclaration('var', [core.types.variableDeclarator(ref, node.right)]),
    );
