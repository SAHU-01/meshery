...
var _default = (0, _helperPluginUtils.declare)((api, opt) => {
  ...
  return {
    ...
    pre() {
      (0, _helperCreateClassFeaturesPlugin.enableFeature)(
        this.file,
        _helperCreateClassFeaturesPlugin.FEATURES.privateIn,
        loose,
      );
    },
    visitor: {
        ...
        if (privateElement.node.type === 'ClassPrivateMethod') {
          if (privateElement.node.static) {
            ...
            const id = getWeakSetId(
              ...
              outerClass,
              outerClass,
              (_outerClass$node$id = outerClass.node.id) == null
                ? void 0
                : _outerClass$node$id.name,
              injectInitialization,
            );
            path.replaceWith(
              template.expression
                .ast`${id}.has(${(0, _helperCreateClassFeaturesPlugin.buildCheckInRHS)(node.right, file)})`,
            );
          }
        } else {
          const id = getWeakSetId(
