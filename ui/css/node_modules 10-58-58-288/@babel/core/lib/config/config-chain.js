...
const loadPresetEnvDescriptors = (0, _caching.makeWeakCacheSync)((preset) =>
...
const loadPresetOverridesDescriptors = (0, _caching.makeWeakCacheSync)((preset) =>
...
const loadPresetOverridesEnvDescriptors = (0, _caching.makeWeakCacheSync)((preset) =>
  (0, _caching.makeStrongCacheSync)((index) =>
    (0, _caching.makeStrongCacheSync)((envName) =>
...
function* buildRootChain(opts, context) {
  ...
  if (typeof opts.configFile === 'string') {
    configFile = yield* (0, _index.loadConfig)(
      opts.configFile,
      context.cwd,
      context.envName,
      context.caller,
  ...
  if ((babelrc === true || babelrc === undefined) && typeof context.filename === 'string') {
    ...
    if (pkgData && babelrcLoadEnabled(context, pkgData, babelrcRoots, babelrcRootsDirectory)) {
      ({ ignore: ignoreFile, config: babelrcFile } = yield* (0, _index.findRelativeConfig)(
        pkgData,
        context.envName,
        context.caller,
      ));
...
const loadProgrammaticChain = makeChainWalker({
  ...
  env: (input, envName) =>
    buildEnvDescriptors(input, 'base', _configDescriptors.createCachedDescriptors, envName),
...
const loadFileOverridesDescriptors = (0, _caching.makeWeakCacheSync)((file) =>
  (0, _caching.makeStrongCacheSync)((index) =>
    buildOverrideDescriptors(
      file,
      file.filepath,
...
const loadFileOverridesEnvDescriptors = (0, _caching.makeWeakCacheSync)((file) =>
  (0, _caching.makeStrongCacheSync)((index) =>
    (0, _caching.makeStrongCacheSync)((envName) =>
      buildOverrideEnvDescriptors(
        file,
...
function buildOverrideDescriptors({ dirname, options }, alias, descriptors, index) {
  ...
  const opts =
    (_options$overrides = options.overrides) == null ? void 0 : _options$overrides[index];
...
function buildOverrideEnvDescriptors({ dirname, options }, alias, descriptors, index, envName) {
  ...
  const override =
    (_options$overrides2 = options.overrides) == null ? void 0 : _options$overrides2[index];
    ...
    if (configIsApplicable(rootOpts, dirname, context, input.filepath)) {
      ...
      (rootOpts.options.overrides || []).forEach((_, index) => {
        ...
        if (configIsApplicable(overrideOps, dirname, context, input.filepath)) {
          ...
          if (
            overrideEnvOpts &&
            configIsApplicable(overrideEnvOpts, dirname, context, input.filepath)
          ) {
            flattenedConfigs.push({
...
function* mergeExtendsChain(chain, opts, dirname, context, files, baseLogger) {
  ...
  const file = yield* (0, _index.loadConfig)(
    opts.extends,
    dirname,
    context.envName,
    context.caller,
