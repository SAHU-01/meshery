...
function isInType(path) {
  switch (path.parent.type) {
    ...
    case 'TSQualifiedName':
      return (
        path.parentPath.findParent((path) => path.type !== 'TSQualifiedName').type !==
    ...
    visitor: {
      ...
      ExportNamedDeclaration(path, state) {
        ...
        if (
          ...
          path.node.specifiers.every(
            (specifier) =>
              t.isExportSpecifier(specifier) && isGlobalType(path, specifier.local.name),
          )
        ) {
          path.remove();
          return;
      ...
      TSImportEqualsDeclaration(path) {
        ...
        path.replaceWith(
          t.variableDeclaration('var', [
            t.variableDeclarator(path.node.id, entityNameToExpr(path.node.moduleReference)),
          ]),
        );
      ...
      [`TSAsExpression${t.tsSatisfiesExpression ? '|TSSatisfiesExpression' : ''}`](path) {
        ...
        do {
          ...
          (t.isTSSatisfiesExpression != null && t.isTSSatisfiesExpression(node))
        );
