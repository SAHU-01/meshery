...
function getMemberExpression(name, itemName) {
  return _core.types.memberExpression(
    ...
    _core.types.identifier(itemName),
  );
}
function handleVariableDeclaration(node, name, hub) {
  ...
  if (declarations.every((declarator) => _core.types.isIdentifier(declarator.id))) {
    for (const declarator of declarations) {
      declarator.init = _core.types.assignmentExpression(
        '=',
  ...
  for (const idName in bindingIdentifiers) {
    assignments.push(
      _core.types.assignmentExpression(
        ...
        _core.types.cloneNode(bindingIdentifiers[idName]),
      ),
    );
  }
...
function handleNested(path, node, parentExport) {
  ...
  const namespaceTopLevel = _core.types.isTSModuleBlock(node.body)
    ? node.body.body
