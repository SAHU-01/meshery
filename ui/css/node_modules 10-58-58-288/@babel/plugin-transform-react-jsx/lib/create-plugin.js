    ...
    } = options;
    {
      ...
      if (RUNTIME_DEFAULT === 'classic') {
        ...
        if (useSpread && useBuiltIns) {
          throw new Error(
            'transform-react-jsx currently only accepts useBuiltIns or useSpread ' + 'but not both',
          );
        }
      }
    }
...
You can set \`throwIfNamespace: false\` to bypass this warning.`);
    ...
    function buildJSXFragmentCall(path, file) {
      ...
      const children = _core.types.react.buildChildren(path.node);
      args.push(
        _core.types.objectExpression(children.length > 0 ? [buildChildrenProperty(children)] : []),
      );
      if (development) {
...
function makeTrace(fileNameIdentifier, lineNumber, column0Based) {
  const fileLineLiteral =
    lineNumber != null ? _core.types.numericLiteral(lineNumber) : _core.types.nullLiteral();
  const fileColumnLiteral =
    column0Based != null ? _core.types.numericLiteral(column0Based + 1) : _core.types.nullLiteral();
