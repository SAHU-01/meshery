...
const collectCompletionsAndVarsVisitor = {
  ...
  'BreakStatement|ContinueStatement'(path, state) {
    ...
    if (label) {
      ...
      path.isBreakStatement() ? state.labellessBreakTargets > 0 : state.labellessContinueTargets > 0
    ) {
      return;
    }
...
function wrapLoopBody(loopPath, captured, updatedBindingsUsages) {
  ...
  for (const [name, updatedUsage] of updatedBindingsUsages) {
    ...
    updater.push(
      _core.types.assignmentExpression(
        ...
        _core.types.identifier(innerName),
      ),
    );
  ...
  const fn = _core.types.functionExpression(
    null,
    closureParams,
    _core.types.toBlock(loopNode.body),
  );
  ...
  const updaterNode =
    updater.length > 0
      ? _core.types.expressionStatement(_core.types.sequenceExpression(updater))
      : null;
  ...
  for (const varPath of state.vars) {
    ...
    if (assign.length > 0) {
      ...
      if (
        !_core.types.isForStatement(varPath.parent, {
          init: varPath.node,
        }) &&
  ...
  if (varNames.length) {
    bodyStmts.push(
      _core.types.variableDeclaration(
        'var',
        varNames.map((name) => _core.types.variableDeclarator(_core.types.identifier(name))),
      ),
    );
  }
