...
function removeUnnecessaryItems(items, overlapping) {
  items.forEach((item) => {
    ...
    (_overlapping$item = overlapping[item]) == null
      ? void 0
      : _overlapping$item.forEach((name) => items.delete(name));
  });
}
function removeUnsupportedItems(items, babelVersion) {
  items.forEach((item) => {
    if (
      has(_availablePlugins.minVersions, item) &&
      _semver.lt(babelVersion, _availablePlugins.minVersions[item])
    ) {
      items.delete(item);
    }
  });
