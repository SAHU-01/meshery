...
function replaceClassWithVar(path) {
  if (path.type === 'ClassDeclaration') {
    ...
    path.insertBefore(
      _core.types.variableDeclaration('let', [_core.types.variableDeclarator(varId)]),
    );
    ...
    const newClassExpr = _core.types.classExpression(
      className && _core.types.identifier(className),
...
function addProxyAccessorsFor(element, originalKey, targetKey, isComputed = false) {
  ...
  const getterBody = _core.types.blockStatement([
    _core.types.returnStatement(
      _core.types.memberExpression(_core.types.thisExpression(), _core.types.cloneNode(targetKey)),
    ),
  ]);
  ...
  if (originalKey.type === 'PrivateName') {
    getter = _core.types.classPrivateMethod(
      'get',
      _core.types.cloneNode(originalKey),
      [],
      getterBody,
      isStatic,
    );
...
function generateDecorationExprs(info) {
  return _core.types.arrayExpression(
    filteredOrderedDecoratorInfo(info).map((el) => {
      const decs =
        el.decorators.length > 1 ? _core.types.arrayExpression(el.decorators) : el.decorators[0];
      const kind = el.isStatic ? el.kind + STATIC : el.kind;
      return _core.types.arrayExpression([
        decs,
        _core.types.numericLiteral(kind),
        el.name,
        ...(el.privateMethods || []),
      ]);
    }),
...
function addCallAccessorsFor(element, key, getId, setId) {
  element.insertAfter(
    _core.types.classPrivateMethod(
      ...
      _core.types.blockStatement([
        _core.types.returnStatement(
          _core.types.callExpression(_core.types.cloneNode(getId), [_core.types.thisExpression()]),
        ),
      ]),
    ),
  );
  element.insertAfter(
    _core.types.classPrivateMethod(
      ...
      _core.types.blockStatement([
        _core.types.expressionStatement(
          _core.types.callExpression(_core.types.cloneNode(setId), [
            _core.types.thisExpression(),
            _core.types.identifier('v'),
          ]),
        ),
...
function movePrivateAccessor(element, key, methodLocalVar, isStatic) {
  ...
  if (element.node.kind === 'set') {
    ...
    block = [
      _core.types.returnStatement(
        _core.types.callExpression(methodLocalVar, [_core.types.thisExpression()]),
      ),
    ];
  }
  element.replaceWith(
...
function staticBlockToIIFE(block) {
  return _core.types.callExpression(
    _core.types.arrowFunctionExpression([], _core.types.blockStatement(block.body)),
    [],
  );
}
...
function transformClass(path, state, constantSuper, version) {
        ...
        if (kind === ACCESSOR) {
          ...
          valuePath.replaceWith(
            _core.types.callExpression(
              _core.types.cloneNode(initId),
              [_core.types.thisExpression(), valuePath.node].filter((v) => v),
            ),
          );
          locals = initId;
          if (isPrivate) {
          ...
          if (kind === GETTER || kind === SETTER) {
            movePrivateAccessor(
              element,
              _core.types.cloneNode(key),
              _core.types.cloneNode(locals),
              isStatic,
            );
          } else {
            const node = element.node;
            path.node.body.body.unshift(
              _core.types.classPrivateProperty(key, _core.types.cloneNode(locals), [], node.static),
            );
  ...
  const classDecorations = _core.types.arrayExpression(
  ...
  if (requiresProtoInit) {
    ...
    const protoInitCall = _core.types.callExpression(_core.types.cloneNode(protoInitLocal), [
      _core.types.thisExpression(),
    ]);
    if (firstFieldPath) {
        ...
        path.traverse({
          CallExpression: {
            exit(path) {
              ...
              path.replaceWith(
                _core.types.callExpression(_core.types.cloneNode(protoInitLocal), [path.node]),
              );
              path.skip();
            },
          },
  ...
  const classInitCall =
    classInitLocal && _core.types.callExpression(_core.types.cloneNode(classInitLocal), []);
  ...
  if (classDecorators) {
    ...
    if (statics.length > 0 || staticBlocks.length > 0) {
      ...
      staticsClass.body.body = [
        _core.types.staticBlock([
          _core.types.toStatement(originalClass, true) ||
            _core.types.expressionStatement(originalClass),
        ]),
        ...statics,
      ];
      ...
      if (constructorBody.length > 0) {
        constructorBody.unshift(
          _core.types.callExpression(_core.types.super(), [_core.types.cloneNode(classIdLocal)]),
        );
        staticsClass.body.body.push(
          _core.types.classMethod(
            ...
            [],
            _core.types.blockStatement([
              _core.types.expressionStatement(_core.types.sequenceExpression(constructorBody)),
            ]),
          ),
        );
      } else {
...
function _default({ assertVersion, assumption }, { loose }, version) {
  var _assumption;
  if (version === '2023-01') {
