...
function applyEnsureOrdering(path) {
  const decorators = (
    path.isClass() ? [path, ...path.get('body.body')] : path.get('properties')
  ).reduce((acc, prop) => acc.concat(prop.node.decorators || []), []);
  const identDecorators = decorators.filter(
    (decorator) => !_core.types.isIdentifier(decorator.expression),
  );
...
function applyObjectDecorators(path, state) {
  ...
  return applyTargetDecorators(
    path,
    state,
    path.node.properties.filter((prop) => prop.type !== 'SpreadElement'),
  );
}
function applyTargetDecorators(path, state, decoratedProps) {
  ...
  const exprs = decoratedProps.reduce(function (acc, node) {
    ...
    const property = _core.types.isLiteral(node.key)
      ? node.key
    ...
    if (
      ...
      const initializer = node.value
        ? _core.types.functionExpression(
            null,
            [],
            _core.types.blockStatement([_core.types.returnStatement(node.value)]),
          )
        : _core.types.nullLiteral();
      node.value = _core.types.callExpression(state.addHelper('initializerWarningHelper'), [
        descriptor,
      ...
      acc.push(
        _core.types.assignmentExpression(
          '=',
          _core.types.cloneNode(descriptor),
          _core.types.callExpression(state.addHelper('applyDecoratedDescriptor'), [
            _core.types.cloneNode(target),
            _core.types.cloneNode(property),
            _core.types.arrayExpression(
              decorators.map((dec) => _core.types.cloneNode(dec.expression)),
            ),
            _core.types.objectExpression([
              _core.types.objectProperty(
                _core.types.identifier('configurable'),
              ...
              _core.types.objectProperty(
                _core.types.identifier('writable'),
                _core.types.booleanLiteral(true),
              ),
  ...
  return _core.types.sequenceExpression([
    _core.types.assignmentExpression('=', _core.types.cloneNode(name), path.node),
...
const visitor = {
      ...
      const [varDeclPath] = path.replaceWithMultiple([
        ...
        _core.types.exportNamedDeclaration(null, [
          _core.types.exportSpecifier(
            _core.types.cloneNode(replacement.declarations[0].id),
  ...
  ClassExpression(path, state) {
    const decoratedClass =
      applyEnsureOrdering(path) || applyClassDecorators(path) || applyMethodDecorators(path, state);
  ...
  AssignmentExpression(path, state) {
    ...
    path.replaceWith(
      _core.types.callExpression(state.addHelper('initializerDefineProperty'), [
        _core.types.cloneNode(path.get('left.object').node),
        _core.types.stringLiteral(
          path.get('left.property').node.name || path.get('left.property').node.value,
        ),
  ...
  CallExpression(path, state) {
    ...
    path.replaceWith(
      _core.types.callExpression(state.addHelper('initializerDefineProperty'), [
        _core.types.cloneNode(path.get('arguments')[0].node),
        _core.types.cloneNode(path.get('arguments')[1].node),
        _core.types.cloneNode(path.get('arguments.2.arguments')[0].node),
        _core.types.cloneNode(path.get('arguments.2.arguments')[1].node),
      ]),
    );
  },
};
