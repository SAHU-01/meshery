...
function buildPrivateNamesNodes(
  ...
  for (const [name, value] of privateNamesMap) {
    ...
    if (privateFieldsAsProperties) {
      init = _core.types.callExpression(state.addHelper('classPrivateFieldLooseKey'), [
        _core.types.stringLiteral(name),
      ]);
    } else if (privateFieldsAsSymbols) {
      init = _core.types.callExpression(_core.types.identifier('Symbol'), [
        _core.types.stringLiteral(name),
      ]);
    } else if (!isStatic) {
...
const privateInVisitor = privateNameVisitorFactory({
  BinaryExpression(path, { file }) {
    ...
    if (isStatic) {
      path.replaceWith(
        _core.template.expression
          .ast`${buildCheckInRHS(right, file)} === ${_core.types.cloneNode(this.classRef)}`,
      );
      return;
    }
    path.replaceWith(
      _core.template.expression
        .ast`${_core.types.cloneNode(id)}.has(${buildCheckInRHS(right, file)})`,
    );
  },
});
const privateNameHandlerSpec = {
  ...
  get(member) {
    ...
    if (isStatic) {
      const helperName =
        isMethod && !isAccessor ? 'classStaticPrivateMethodGet' : 'classStaticPrivateFieldSpecGet';
    ...
    if (isMethod) {
      ...
      return _core.types.callExpression(file.addHelper('classPrivateMethodGet'), [
        this.receiver(member),
        _core.types.cloneNode(id),
  ...
  boundGet(member) {
    ...
    return _core.types.callExpression(
      _core.types.memberExpression(this.get(member), _core.types.identifier('bind')),
      [this.receiver(member)],
    );
  },
  set(member, value) {
    ...
    if (isStatic) {
      const helperName =
        isMethod && !isAccessor ? 'classStaticPrivateMethodSet' : 'classStaticPrivateFieldSpecSet';
      return _core.types.callExpression(file.addHelper(helperName), [
        this.receiver(member),
        _core.types.cloneNode(classRef),
        _core.types.cloneNode(id),
  ...
  destructureSet(member) {
    ...
    return _core.types.memberExpression(
      _core.types.callExpression(file.addHelper('classPrivateFieldDestructureSet'), [
        this.receiver(member),
        _core.types.cloneNode(id),
      ]),
      _core.types.identifier('value'),
  ...
  return inheritPropComments(
    _core.types.expressionStatement(
      _core.types.assignmentExpression(
        '=',
        _core.types.memberExpression(ref, key, computed || _core.types.isLiteral(key)),
        value,
      ),
    ),
    prop,
...
const thisContextVisitor = _core.traverse.visitors.merge([
  {
    ThisExpression(path, state) {
      const parent = path.findParent(
        (path) =>
          !(0, _helperSkipTransparentExpressionWrappers.isTransparentExprWrapper)(path.node),
      );
      if (
        _core.types.isUnaryExpression(parent.node, {
          operator: 'delete',
        })
      ) {
...
function replaceThisContext(
  ...
  const replacer = new _helperReplaceSupers.default({
    ...
    getObjectRef() {
      state.needsClassRef = true;
      return (_core.types.isStaticBlock != null && _core.types.isStaticBlock(path.node)) ||
        path.node.static
        ? ref
        : _core.types.memberExpression(ref, _core.types.identifier('prototype'));
    },
  });
  replacer.replace();
  ...
  if (
    ...
    state.classRef.name !== (innerBindingRef == null ? void 0 : innerBindingRef.name)
  ) {
...
function buildFieldsInitNodes(
  ...
  for (const prop of props) {
    ...
    const isStatic =
      !(_core.types.isStaticBlock != null && _core.types.isStaticBlock(prop.node)) &&
      prop.node.static;
    ...
    switch (true) {
      case isStaticBlock: {
        ...
        if (blockBody.length === 1 && _core.types.isExpressionStatement(blockBody[0])) {
          ...
          staticNodes.push(
            _core.types.inheritsComments(
              _core.template.statement.ast`(() => { ${blockBody} })()`,
              prop.node,
            ),
          );
        }
      ...
      case isInstance && isPrivate && isMethod && privateFieldsAsProperties:
        instanceNodes.unshift(
          buildPrivateMethodInitLoose(_core.types.thisExpression(), prop, privateNamesMap),
        );
        pureStaticNodes.push(
          buildPrivateMethodDeclaration(prop, privateNamesMap, privateFieldsAsProperties),
        );
        break;
      case isInstance && isPrivate && isMethod && !privateFieldsAsProperties:
        instanceNodes.unshift(
  ...
  return {
    ...
    wrapClass(path) {
      ...
      if (injectSuperRef) {
        ...
        path.set(
          'superClass',
          _core.types.assignmentExpression('=', injectSuperRef, path.node.superClass),
        );
      }
      ...
      if (path.isClassExpression()) {
        ...
        path.replaceWith(
          _core.types.assignmentExpression('=', _core.types.cloneNode(ref), path.node),
        );
