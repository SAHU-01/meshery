...
var index = helperPluginUtils.declare((api, opts) => {
  ...
  const objectRestNoSymbols =
    (_api$assumption2 = api.assumption('objectRestNoSymbols')) != null ? _api$assumption2 : loose;
  const pureGetters =
    (_api$assumption3 = api.assumption('pureGetters')) != null ? _api$assumption3 : loose;
  const setSpreadProperties =
    (_api$assumption4 = api.assumption('setSpreadProperties')) != null ? _api$assumption4 : loose;
  ...
  function createObjectRest(path, file, objRef) {
    ...
    const { keys, allLiteral, hasTemplateLiteral } = extractNormalizedKeys(path.node);
    if (keys.length === 0) {
      return [
    ...
    if (!allLiteral) {
      keyExpression = core.types.callExpression(
        core.types.memberExpression(core.types.arrayExpression(keys), core.types.identifier('map')),
    ...
    return [
      ...
      core.types.callExpression(
        file.addHelper(`objectWithoutProperties${objectRestNoSymbols ? 'Loose' : ''}`),
    ...
    visitor: {
        ...
        visitRestElements(path.get('id'), (path) => {
          ...
          if (
            ...
            originalPath.replaceWith(
              core.types.variableDeclarator(originalPath.node.id, core.types.cloneNode(initRef)),
            );
            return;
          }
      ...
      ExportNamedDeclaration(path) {
        ...
        for (const name of Object.keys(path.getOuterBindingIdentifiers(true))) {
          specifiers.push(
            core.types.exportSpecifier(core.types.identifier(name), core.types.identifier(name)),
          );
        }
      ...
      ForXStatement(path) {
        ...
        if (!core.types.isVariableDeclaration(left)) {
          ...
          body.body.unshift(
            core.types.expressionStatement(
              core.types.assignmentExpression('=', left, core.types.cloneNode(temp)),
            ),
          );
        } else {
