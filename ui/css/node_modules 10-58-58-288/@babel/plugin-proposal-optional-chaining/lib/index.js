...
function transform(path, { pureGetters, noDocumentAll }) {
  ...
  while (optionalPath.isOptionalMemberExpression() || optionalPath.isOptionalCallExpression()) {
    ...
    if (optionalPath.isOptionalMemberExpression()) {
      ...
      optionalPath = helperSkipTransparentExpressionWrappers.skipTransparentExprWrappers(
        optionalPath.get('object'),
      );
  ...
  for (let i = optionals.length - 1; i >= 0; i--) {
    ...
    const chain =
      helperSkipTransparentExpressionWrappers.skipTransparentExprWrapperNodes(chainWithTypes);
    let ref;
    let check;
    if (
      isCall &&
    ...
    if (i === 0 && parentIsCall) {
      var _baseRef;
      const object = helperSkipTransparentExpressionWrappers.skipTransparentExprWrapperNodes(
        replacement.object,
      );
      let baseRef;
...
var index = helperPluginUtils.declare((api, options) => {
  ...
  const noDocumentAll =
    (_api$assumption = api.assumption('noDocumentAll')) != null ? _api$assumption : loose;
  const pureGetters =
    (_api$assumption2 = api.assumption('pureGetters')) != null ? _api$assumption2 : loose;
  return {
