...
function transformClass(path, file, builtinClasses, isLoose, assumptions, supportUnicodeId) {
  ...
  const findThisesVisitor = _core.traverse.visitors.merge([
  ...
  function maybeCreateConstructor() {
    ...
    classState.path
      ...
      .unshiftContainer(
        'body',
        _core.types.classMethod('constructor', _core.types.identifier('constructor'), params, body),
      );
  }
  function buildBody() {
      ...
      if (classState.methods[placement].list.length) {
        props[placement] = classState.methods[placement].list.map((desc) => {
          ...
          for (const kind of ['get', 'set', 'value']) {
            if (desc[kind] != null) {
              obj.properties.push(
                _core.types.objectProperty(_core.types.identifier(kind), desc[kind]),
              );
            }
          }
          return obj;
  ...
  function wrapSuperCall(bareSuper, superRef, thisRef, body) {
    ...
    if (assumptions.superIsCallableConstructor) {
      ...
      call = (0, _helperOptimiseCallExpression.default)(
        _core.types.cloneNode(classState.superFnId),
  ...
  function verifyConstructor() {
    ...
    for (const thisPath of classState.superThises) {
      ...
      thisPath.replaceWith(
        _core.types.callExpression(classState.file.addHelper('assertThisInitialized'), [thisRef()]),
      );
    }
    const bareSupers = [];
    path.traverse(
    ...
    if (classState.isLoose) {
      wrapReturn = (returnArg) => {
        const thisExpr = _core.types.callExpression(
    ...
    if (!bodyPaths.length || !bodyPaths.pop().isReturnStatement()) {
      body.pushContainer(
        'body',
        _core.types.returnStatement(guaranteedSuperBeforeFinish ? thisRef() : wrapReturn()),
      );
    }
  ...
  function pushMethod(node, path) {
    ...
    const key =
      _core.types.isNumericLiteral(node.key) || _core.types.isBigIntLiteral(node.key)
    ...
    if (_core.types.isStringLiteral(key)) {
      if (node.kind === 'method') {
        var _nameFunction;
        fn =
          (_nameFunction = (0, _helperFunctionName.default)(
            {
              id: key,
              node: node,
              scope,
  ...
  function processMethod(node, scope) {
    if (assumptions.setClassMethods && !node.decorators) {
      ...
      let func = _core.types.functionExpression(
        null,
        node.params,
        node.body,
        node.generator,
      ...
      if (_core.types.isStringLiteral(key)) {
        var _nameFunction2;
        func =
          (_nameFunction2 = (0, _helperFunctionName.default)(
            {
              node: func,
              id: key,
              scope,
      ...
      classState.body.unshift(
        _core.types.variableDeclaration('var', [
          _core.types.variableDeclarator(
            superFnId,
            _core.types.callExpression((0, _inlineCreateSuperHelpers.default)(classState.file), [
              _core.types.cloneNode(classState.classRef),
            ]),
          ),
        ]),
    ...
    classState.body.unshift(
      _core.types.expressionStatement(
        _core.types.callExpression(
          classState.file.addHelper(classState.isLoose ? 'inheritsLoose' : 'inherits'),
          [_core.types.cloneNode(classState.classRef), _core.types.cloneNode(classState.superName)],
        ),
      ),
    );
  }
  ...
  function classTransformer(path, file, builtinClasses, isLoose) {
    ...
    if (!assumptions.noClassCalls) {
      constructorBody.body.unshift(
        _core.types.expressionStatement(
          _core.types.callExpression(classState.file.addHelper('classCallCheck'), [
            _core.types.thisExpression(),
            _core.types.cloneNode(classState.classRef),
          ]),
        ),
