...
var _default = (0, _helperPluginUtils.declare)((api, options) => {
  ...
  const setComputedProperties =
    (_api$assumption = api.assumption('setComputedProperties')) != null
      ? _api$assumption
      : options.loose;
  ...
  function pushAccessorDefine({ body, computedProps, initPropExpression, objId, state }, prop) {
    ...
    if (computedProps.length === 1) {
      ...
      body.push(
        _core.types.expressionStatement(
          buildDefineAccessor(state, kind, _core.types.cloneNode(objId), key, value),
        ),
      );
    }
  }
  ...
  function pushComputedPropsSpec(info) {
    ...
    for (const prop of computedProps) {
      ...
      if (_core.types.isObjectMethod(prop) && (prop.kind === 'get' || prop.kind === 'set')) {
        ...
        const value = getValue(prop);
        if (computedProps.length === 1) {
          return _core.types.callExpression(state.addHelper('defineProperty'), [
