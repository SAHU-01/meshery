        ...
        !theme.vars &&
          theme.palette.mode === 'dark' && {
            backgroundImage: 'linear-gradient('
              .concat(alpha('#fff', getOverlayAlpha(ownerState.elevation)), ', ')
              .concat(alpha('#fff', getOverlayAlpha(ownerState.elevation)), ')'),
          },
        theme.vars && {
          backgroundImage:
...
var Paper = /*#__PURE__*/ React.forwardRef(function Paper(inProps, ref) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (theme.shadows[elevation] === undefined) {
      console.error(
        [
          'MUI: The elevation provided <Paper elevation={'.concat(
            elevation,
  ...
  ? (Paper.propTypes /* remove-proptypes */ = {
      ...
      elevation: chainPropTypes(integerPropType, function (props) {
        ...
        if (elevation > 0 && variant === 'outlined') {
          return new Error(
            'MUI: Combining `elevation={'
              .concat(elevation, '}` with `variant="')
              .concat(
                variant,
                '"` has no effect. Either use `elevation={0}` or use a different `variant`.',
              ),
          );
        }
        return null;
