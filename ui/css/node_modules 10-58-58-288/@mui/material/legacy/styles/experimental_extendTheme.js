...
var defaultDarkOverlays = _toConsumableArray(Array(25)).map(function (_, index) {
  ...
  return 'linear-gradient(rgba(255 255 255 / '
    .concat(overlay, '), rgba(255 255 255 / ')
    .concat(overlay, '))');
});
...
function setColorChannel(obj, key) {
  if (!(''.concat(key, 'Channel') in obj)) {
    ...
    obj[''.concat(key, 'Channel')] = safeColorChannel(
      toRgb(obj[key]),
      "MUI: Can't create `palette."
        .concat(key, 'Channel` because `palette.')
        .concat(
          key,
...
export default function extendTheme() {
  ...
  var _options$colorSchemes = options.colorSchemes,
    ...
    shouldSkipGeneratingVar =
      _options$shouldSkipGe === void 0 ? defaultShouldSkipGeneratingVar : _options$shouldSkipGe,
    input = _objectWithoutProperties(options, [
      'colorSchemes',
      'cssVarPrefix',
      'shouldSkipGeneratingVar',
    ]);
  ...
  Object.keys(theme.colorSchemes).forEach(function (key) {
    ...
    if (key === 'light') {
      ...
      setColor(
        palette.Skeleton,
        'bg',
        'rgba('.concat(setCssVarColor('palette-text-primaryChannel'), ' / 0.11)'),
      );
      ...
      setColor(
        palette.Skeleton,
        'bg',
        'rgba('.concat(setCssVarColor('palette-text-primaryChannel'), ' / 0.13)'),
      );
  ...
  theme.unstable_sxConfig = _extends(
    {},
    defaultSxConfig,
