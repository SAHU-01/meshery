...
var defaultIsActiveElementInListbox = function defaultIsActiveElementInListbox(listboxRef) {
  ...
  return (
    ...
    ((_listboxRef$current$p = listboxRef.current.parentElement) == null
      ? void 0
...
export function useAutocomplete(props) {
  var _props$unstable_isAct = props.unstable_isActiveElementInListbox,
    unstable_isActiveElementInListbox =
      _props$unstable_isAct === void 0 ? defaultIsActiveElementInListbox : _props$unstable_isAct,
      ...
      if (process.env.NODE_ENV !== 'production') {
        var erroneousReturn =
          optionLabel === undefined
            ? 'undefined'
            : ''.concat(_typeof(optionLabel), ' (').concat(optionLabel, ')');
        console.error(
          'MUI: The `getOptionLabel` method of '
            .concat(componentName, ' returned ')
            .concat(erroneousReturn, ' instead of a string for ')
            .concat(JSON.stringify(option), '.'),
        );
      }
  ...
  var resetInputValue = React.useCallback(
    ...
    ? filterOptions(
        options.filter(function (option) {
          if (
            ...
            (multiple ? value : [value]).some(function (value2) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (value !== null && !freeSolo && options.length > 0) {
      ...
      if (missingValue.length > 0) {
        console.warn(
          [
            'MUI: The value provided to '.concat(componentName, ' is invalid.'),
            'None of the options match with `'.concat(
  ...
  var setHighlightedIndex = useEventCallback(function (_ref2) {
    ...
    var prev = listboxRef.current.querySelector(
      '[role="option"].'.concat(unstable_classNamePrefix, '-focused'),
    );
    if (prev) {
    ...
    if (
      ...
      (multiple
        ? value.length === previousProps.value.length &&
          previousProps.value.every(function (val, i) {
      ...
      function () {
        if (!inputRef.current || inputRef.current.nodeName !== 'INPUT') {
          if (inputRef.current && inputRef.current.nodeName === 'TEXTAREA') {
            console.warn(
              [
                'A textarea element was provided to '.concat(
                  componentName,
                  ' where input was expected.',
                ),
            ...
            console.error(
              [
                'MUI: Unable to find the input element. It was resolved to '.concat(
                  inputRef.current,
                  ' while an HTMLInputElement was expected.',
                ),
                'Instead, '.concat(componentName, ' expects an input element.'),
                '',
  ...
  var selectNewValue = function selectNewValue(event, option) {
    var reasonProp =
      arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'selectOption';
    ...
    if (multiple) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (matches.length > 1) {
          console.error(
            [
              'MUI: The `isOptionEqualToValue` method of '.concat(
                componentName,
                ' does not handle the arguments correctly.',
              ),
              'The component expects a single value to match a given option but found '.concat(
                matches.length,
                ' matches.',
              ),
            ].join('\n'),
      ...
      if (acc.length > 0 && acc[acc.length - 1].group === group) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (indexBy.get(group) && !warn) {
            console.warn(
              'MUI: The options provided combined with the `groupBy` method of '.concat(
                componentName,
  ...
  return {
    ...
    getOptionProps: function getOptionProps(_ref5) {
      ...
      return {
        key:
          (_getOptionKey = getOptionKey == null ? void 0 : getOptionKey(option)) != null
            ? _getOptionKey
