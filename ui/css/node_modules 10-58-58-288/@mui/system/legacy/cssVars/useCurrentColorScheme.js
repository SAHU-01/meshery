...
export default function useCurrentColorScheme(options) {
  var _options$defaultMode = options.defaultMode,
    ...
    storageWindow =
      _options$storageWindo === void 0
        ? typeof window === 'undefined'
          ? undefined
          : window
        : _options$storageWindo;
  var joinedColorSchemes = supportedColorSchemes.join(',');
  var _React$useState = React.useState(function () {
      var initialMode = initializeValue(modeStorageKey, defaultMode);
      var lightColorScheme = initializeValue(
        ''.concat(colorSchemeStorageKey, '-light'),
        defaultLightColorScheme,
      );
      var darkColorScheme = initializeValue(
        ''.concat(colorSchemeStorageKey, '-dark'),
  ...
  var setColorScheme = React.useCallback(
    function (value) {
      if (!value) {
        setState(function (currentState) {
          try {
        ...
        setState(function (currentState) {
          ...
          if (newLightColorScheme) {
            if (!joinedColorSchemes.includes(newLightColorScheme)) {
              console.error(
                '`'.concat(newLightColorScheme, '` does not exist in `theme.colorSchemes`.'),
              );
            } else {
          ...
          if (newDarkColorScheme) {
            if (!joinedColorSchemes.includes(newDarkColorScheme)) {
              console.error(
                '`'.concat(newDarkColorScheme, '` does not exist in `theme.colorSchemes`.'),
              );
            } else {
  ...
  var handleMediaQuery = React.useCallback(
    function (event) {
