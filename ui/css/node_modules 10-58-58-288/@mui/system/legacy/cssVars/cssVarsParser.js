...
export var walkObjectDeep = function walkObjectDeep(obj, callback, shouldSkipPaths) {
  function recurse(object) {
    ...
    Object.entries(object).forEach(function (_ref) {
      ...
      if (
        !shouldSkipPaths ||
        (shouldSkipPaths && !shouldSkipPaths([].concat(_toConsumableArray(parentKeys), [key])))
      ) {
        if (value !== undefined && value !== null) {
          if (_typeof(value) === 'object' && Object.keys(value).length > 0) {
            recurse(
              value,
              [].concat(_toConsumableArray(parentKeys), [key]),
              Array.isArray(value) ? [].concat(_toConsumableArray(arrayKeys), [key]) : arrayKeys,
            );
          } else {
...
var getCssValue = function getCssValue(keys, value) {
  if (typeof value === 'number') {
    if (
      ['lineHeight', 'fontWeight', 'opacity', 'zIndex'].some(function (prop) {
      ...
      if (typeof value === 'string' || typeof value === 'number') {
        if (!shouldSkipGeneratingVar || !shouldSkipGeneratingVar(keys, value)) {
          ...
          assignNestedKeys(
            varsWithDefaults,
            keys,
            'var('.concat(cssVar, ', ').concat(value, ')'),
            arrayKeys,
          );
        }
      }
