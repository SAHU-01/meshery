  ...
  if (['rgb', 'rgba', 'hsl', 'hsla', 'color'].indexOf(type) === -1) {
    throw new Error(
      process.env.NODE_ENV !== 'production'
        ? 'MUI: Unsupported `'.concat(
            color,
            '` color.\nThe following formats are supported: #nnn, #nnnnnn, rgb(), rgba(), hsl(), hsla(), color().',
          )
        : _formatMuiErrorMessage(9, color),
    );
  }
    ...
    if (['srgb', 'display-p3', 'a98-rgb', 'prophoto-rgb', 'rec-2020'].indexOf(colorSpace) === -1) {
      throw new Error(
        process.env.NODE_ENV !== 'production'
          ...
          : _formatMuiErrorMessage(10, colorSpace),
      );
    }
  } else {
    values = values.split(',');
...
export function blend(background, overlay, opacity) {
  ...
  var blendChannel = function blendChannel(b, o) {
    return Math.round(
      Math.pow(Math.pow(b, 1 / gamma) * (1 - opacity) + Math.pow(o, 1 / gamma) * opacity, gamma),
    );
  };
