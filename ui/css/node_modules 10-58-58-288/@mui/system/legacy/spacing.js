    ...
    return function (abs) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (typeof abs !== 'number') {
          console.error(
            'MUI: Expected '
              .concat(propName, ' argument to be a number or a string, got ')
              .concat(abs, '.'),
          );
        }
      }
    ...
    return function (abs) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        if (!Number.isInteger(abs)) {
          console.error(
            [
              'MUI: The `theme.'.concat(
                themeKey,
                '` array type cannot be combined with non integer values.',
              ) +
                'You should either use an integer value that can be used as index, or define the `theme.'.concat(
                  themeKey,
                  '` as a number.',
                ),
            ].join('\n'),
          ...
          console.error(
            [
              ...
              ''
                .concat(abs, ' > ')
                .concat(themeSpacing.length - 1, ', you need to add the missing values.'),
            ].join('\n'),
          );
        }
      }
...
margin.propTypes =
  process.env.NODE_ENV !== 'production'
    ? marginKeys.reduce(function (obj, key) {
...
padding.propTypes =
  process.env.NODE_ENV !== 'production'
    ? paddingKeys.reduce(function (obj, key) {
        obj[key] = responsivePropType;
        return obj;
      }, {})
    : {};
