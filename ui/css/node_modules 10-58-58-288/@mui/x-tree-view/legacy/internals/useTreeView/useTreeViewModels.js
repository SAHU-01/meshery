...
export var useTreeViewModels = function useTreeViewModels(plugins, props) {
  ...
  var models = Object.fromEntries(
    Object.entries(modelsRef.current).map(function (_ref3) {
            ...
            console.error(
              [
                'MUI: A component is changing the '
                  .concat(model.isControlled ? '' : 'un', 'controlled ')
                  .concat(modelName, ' state of TreeView to be ')
                  .concat(model.isControlled ? 'un' : '', 'controlled.'),
                'Elements should not switch from uncontrolled to controlled (or vice versa).',
                'Decide between using a controlled or uncontrolled '.concat(modelName, ' ') +
      ...
      React.useEffect(
        function () {
          if (!model.isControlled && defaultValue !== defaultProp) {
            console.error(
              [
                'MUI: A component is changing the default '.concat(
                  modelName,
                  ' state of an uncontrolled TreeView after being initialized. ',
